...
@Component({
  selector: 'app-IncomingFeed_Create',
  templateUrl: './IncomingFeed_Create.component.html',
  styleUrls: ['./IncomingFeed_Create.component.scss'],
  encapsulation: ViewEncapsulation.None,
  providers: [ApiService, ToasterService]
})
export class IncomingFeed_CreateComponent implements OnInit {
  companies = [{name: 'test'}];
  selectedCompany;
  ...
  constructor(private api: ApiService, private toasterService: ToasterService) {
    ...
    this.getStocks();
    this.selectedStock = {id: '', name: '', life_cycle: '', metric: '', category: '', sapstockcode: '', skt: ''};
    this.getBatches();
  ...
  tuketim = () => {
    for (let i = 0; i < this.num; i++) {
      if (!(this.kutucukSay[i] == null) && !(this.tankSay[i] == null) && !(this.fishOP[i] == null)) {
