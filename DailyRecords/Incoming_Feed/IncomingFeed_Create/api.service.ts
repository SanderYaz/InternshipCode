...
export class ApiService {
  ...
  getOneStocklevels(id): Observable<any> {
    return this.http.get(this.baseurl + '/stocklevels/' + id,
      {headers: this.httpHeaders});
  }

  updateStocklevels(stocklevels): Observable<any> {
    const body = {
      company: stocklevels.company, facility: stocklevels.facility, unit: stocklevels.unit, tank: stocklevels.tank,
      stock: stocklevels.stock, batchcode: stocklevels.batchcode, quantity: stocklevels.quantity,
      category: stocklevels.category
    };
    return this.http.put(this.baseurl + '/stocklevels/' + stocklevels.id, body,
      {headers: this.httpHeaders});
  }

  createStocklevels(stocklevels): Observable<any> {
    const body = {
      company: stocklevels.company,
      facility: stocklevels.facility,
      unit: stocklevels.unit,
      tank: stocklevels.tank,
      stock: stocklevels.stock,
      batchcode: stocklevels.batchcode,
      id: stocklevels.id,
      quantity: stocklevels.quantity,
      metric: stocklevels.metric,
      category: stocklevels.category,
      sapstockcode: stocklevels.sapstockcode,
      skt: stocklevels.skt
    };
  ...
  createConsumptions(consumptions, tanks, slevel, stocks, quantity, OP, batchs): Observable<any> {
    const body = {
      id: consumptions.id,
  ...
  updateStocklvl(stocklevels): Observable<any> {
    const body = {quantity: stocklevels.quantity,
    };
    return this.http.put(this.baseurl + '/stocklevels/' + stocklevels.id, body,
      {headers: this.httpHeaders});
  }

  deleteStocklevels(id): Observable<any> {
    return this.http.delete(this.baseurl + '/stocklevels/' + id,
    );
  }
}
